...
1. 新建活动后与策划助手对话，逐步沟通活动安排；除活动名称外没有必填项。未配置模型接口时，可改用需求表单。
2. 信息足够后助手会询问是否生成方案；确认后助手读取生成技能，在该活动的 workspace 中写入 `plan.md`。页面出现文件卡片，点击弹出预览，可继续对话修改，或在预览窗口点击“开始实施”。实施后与策划助手的对话即关闭。
3. 开始实施即确认策划并创建报名入口；生成宣传，预览并确认文案和海报。确认后开放本站报名。
