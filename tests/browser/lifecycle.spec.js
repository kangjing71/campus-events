...
test('complete organizer and participant lifecycle', async ({ page, browser, request }) => {
  ...
  await expect(participant.getByRole('button',{name:'现场问题'})).toBeDisabled();
  await expect(participant.getByRole('button',{name:'复盘反馈'})).toBeDisabled();
  await participant.getByRole('button',{name:'提交报名'}).click();
  await expect(participant.locator('#toast')).toContainText('报名成功');
  await expect(participant.getByLabel('姓名 *')).toBeVisible();
  ...
  await expect(page.getByText('签到入场')).toBeVisible();
  await page.getByRole('button',{name:'开始活动',exact:true}).click();
  await page.getByRole('button',{name:'确认执行'}).click();
  await page.getByTitle('为 李同学 签到').click();
  await expect(page.getByRole('cell',{name:'已签到'})).toBeVisible();
  await participant.reload();
  await expect(participant.getByRole('button',{name:'活动报名'})).toBeDisabled();
  await expect(participant.getByRole('button',{name:'复盘反馈'})).toBeDisabled();
  ...
  await page.getByRole('button',{name:'刷新活动数据'}).click();
  await expect(page.locator('.feedback-item')).toContainText('希望多留一点讨论时间');
  await expect(page.locator('.feedback-item')).toContainText('请现场工作人员及时关注');
