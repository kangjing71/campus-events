...
const pages = [['overview','layout-dashboard','活动总览'],['plan','clipboard-list','活动策划'],['registration','users','报名管理'],['publicity','megaphone','宣传中心'],['onsite','radio','现场执行'],['review','chart-no-axes-combined','活动复盘']];
...
function briefProgress(b) {
  const items=requiredFields.map(([k,label])=>({label,v:briefValue(b,k)})).filter(i=>i.v);
  if(!items.length)return '';
  return `<div class="brief-progress"><div class="brief-progress-head"><h3>已掌握的活动信息</h3><span class="small muted">${items.length} 项</span></div><div class="brief-fields">${items.map(i=>`<div class="brief-field"><span class="brief-label">${i.label}</span><span class="brief-value">${esc(i.v)}</span></div>`).join('')}</div></div>`;
}
function planDraft() {
  ...
  const welcome=`<div class="chat-welcome"><h2>一起策划「${esc(e.brief.name)}」</h2><p>和我聊聊这场活动的安排，时间、地点、规模、预算都可以。信息足够时我会生成策划方案供你确认。</p><div class="chips">${chips.map(([label,text])=>`<button class="chip" data-chat-example="${esc(text)}">${label}</button>`).join('')}</div></div>${briefProgress(e.brief)}`;
  const card=e.plan_md?`<button class="file-chip" data-action="preview-plan">${icon('file-text')}<span class="file-chip-main"><strong>plan.md</strong><span class="small muted">策划方案 · 第 ${e.revision} 版 · ${(new Blob([e.plan_md]).size/1024).toFixed(1)} KB · 点击预览</span></span>${icon('chevron-right')}</button>`:'';
  const placeholder=msgs.length?'继续补充或修改活动信息……':`例如：${e.brief.name}计划下周五晚 7 点在学生活动中心举办，预计 100 人，预算 1200 元`;
...
function planView() {
  ...
  if (['DRAFT','WAITING_PLAN_CONFIRMATION'].includes(e.state)) return planDraft();
  if (!p) return `<div class="section-head"><h2>策划方案 <span class="small muted">plan.md · 版本 ${e.revision}</span></h2>${badge(e.state)}</div><div class="panel plan-md">${md(e.plan_md||'')}</div>`;
...
function publicityView(recap=false) {
  ...
  if(!p){const ready=['PLAN_CONFIRMED','REGISTRATION_OPEN'].includes(e.state);return empty('宣传内容尚未生成',ready?'报名渠道已就绪，可生成宣传内容。':'确认策划方案后进入宣传准备。',ready?button('生成宣传内容','publicity',true,'sparkles'):'');}
  const editable=recap?e.state==='WAITING_RECAP_CONFIRMATION':['WAITING_PUBLICITY_CONFIRMATION','REGISTRATION_OPEN'].includes(e.state);
  const copy=p[pubTab]??p.group??'';
  return `${recap?(editable?'<div class="inline-note warning">文案待确认。确认后可复制文案与下载海报。</div>':''):''}<div class="section-head"><h2>${recap?'活动总结宣传':'活动宣传素材'}</h2><div class="actions">${editable?button('修改文案',recap?'edit-recap':'edit-copy',false,'pencil')+(recap?button('确认总结并归档','confirm_recap',true,'check'):''):''}</div></div><div class="split"><div><div class="tabbar">${[['article','公众号'],['moments','朋友圈'],['xiaohongshu','小红书']].map(([id,t])=>`<button class="${pubTab===id?'active':''}" data-tab="${id}">${t}</button>`).join('')}</div><div class="plan-md">${md(copy.replaceAll(e.registration_path,link()))}</div><div class="form-actions">${button('复制文案',recap?'copy-recap':'copy-content',false,'copy')}</div></div><div><div class="poster"><div class="poster-photo"></div><span class="eyebrow">CAMPUS TOGETHER / ${recap?'活动回顾':'校园活动'}</span><h2>${esc(e.brief.name)}</h2><p>${recap?`${e.metrics.registration} 人报名 · ${e.metrics.attendance} 人到场` :esc(e.brief.objective)}</p><p>${fmt(e.brief.date)}<br>${esc(e.brief.location)}<br>${esc(e.brief.organizer)}</p><canvas id="qr" aria-label="活动入口二维码"></canvas><span class="small">${recap?'活动参与者入口':'扫码报名 · 期待相遇'}</span></div><div class="form-actions">${button('下载海报',recap?'poster-recap':'poster',false,'download')}</div></div></div>`;
}
function registrationView() {
  if (!event.registration_path) return empty('报名入口尚未创建','确认策划后，由系统自动创建入口。');
  return `<div class="link-box">${icon('link')}<a target="_blank" rel="noopener" href="${esc(link())}">${esc(link())}</a>${button('复制链接','copy-link',false,'copy')}${button('二维码','show-qr',false,'qr-code')}</div>${event.state==='PLAN_CONFIRMED'||event.state==='WAITING_PUBLICITY_CONFIRMATION'?'<div class="inline-note warning">报名入口已创建，宣传确认后开放提交。</div>':''}<div class="section-head"><h2>报名人员 <span class="small muted">${event.registrations.length} 人</span></h2><div class="actions nowrap-actions"><input class="search" id="search" aria-label="搜索报名人员" placeholder="搜索姓名、邮箱、学院">${button('导出名单','export',false,'download')}</div></div><div id="registration-table">${registrationTable(event.registrations)}</div><div class="section"><h2>近 7 天报名趋势</h2>${trend()}</div>`;
}
function registrationTable(rows) {
  const fields=event.questionnaire?.fields||[];
  const labelMap=Object.fromEntries(fields.map(f=>[f.key,f.label]));
  const extraKeys=fields.map(f=>f.key).filter(k=>!['name','contact','email','needs','question'].includes(k));
  const legacyKeys=[['college','学院'],['grade','年级'],['source','来源']];
  const extra=r=>{
    const items=r.answers?extraKeys.map(k=>[labelMap[k]||k,r.answers[k]]).filter(([,v])=>v):legacyKeys.map(([k,l])=>[l,r[k]]).filter(([,v])=>v);
    return items.map(([l,v])=>`${esc(l)}：${esc(v)}`).join('<br>');
  };
  return rows.length?`<div class="table-wrap"><table><thead><tr><th>参与者</th><th>联系方式</th><th>报名信息</th><th>需求与提问</th><th>状态</th><th>操作</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${esc(r.contact||r.email||'')}</td><td class="wrap">${extra(r)||'<span class="muted">—</span>'}</td><td class="wrap">${esc(r.needs||'')}${r.question?`<br><span class="muted">问：${esc(r.question)}</span>`:''}${r.answer?`<br>答：${esc(r.answer)}`:''}${!r.needs&&!r.question?'<span class="muted">—</span>':''}</td><td><span class="badge ${r.checked_at?'':'gray'}">${r.checked_at?(r.late?'迟到已签到':'已签到'):['FEEDBACK','WAITING_REVIEW_CONFIRMATION','WAITING_RECAP_CONFIRMATION','COMPLETED'].includes(event.state)?'未到':'已报名'}</span></td><td>${event.state==='LIVE'&&!r.checked_at?`<button class="icon-btn" title="为 ${esc(r.name)} 签到" data-checkin="${r.id}">${icon('scan-line')}</button>`:''}${r.question&&['REGISTRATION_OPEN','LIVE','FEEDBACK'].includes(event.state)?`<button class="icon-btn" title="回复提问" data-answer="${r.id}">${icon('message-square')}</button>`:''}</td></tr>`).join('')}</tbody></table></div>`:empty('暂无报名人员','报名成功后，参与者名单会显示在这里。');
}
function trend() {
...
function onsiteView() {
  const e=event,p=e.plan,pend=e.pending_adjustment;
  const live=e.state==='LIVE';
  const tl=e.onsite_timeline||(p&&p.timeline)||null;
  const current=live&&tl?[...tl].reverse().find(t=>new Date(t.time)<=new Date()):null;
  const next=live&&tl?tl.find(t=>new Date(t.time)>new Date()):null;
  const issues=e.feedback_pool.filter(f=>f.phase==='live');
  const a=e.onsite_analysis;
  const agentBlock=`<div class="section"><div class="section-head"><h2>现场情况分析</h2><span class="small muted">现场 Agent</span></div>${live&&!pend?`<form id="onsite-agent-form"><div class="field"><label for="onsite-message">现场情况</label><textarea id="onsite-message" name="message" maxlength="6000" placeholder="补充工作人员观察到的情况，例如嘉宾预计晚到 20 分钟"></textarea></div><div class="form-actions"><button class="primary" type="submit">${icon('sparkles')}分析现场并提出建议</button></div></form>`:''}${a?`<p class="prose analysis-summary">${esc(a.summary)}</p>${a.issues.map(i=>`<div class="feedback-item"><strong>${esc(i.topic)}</strong><p>${esc(i.suggestion)}</p><p class="small muted">依据：${i.evidence_ids.map(esc).join('、')||'工作人员情况 / 时间线'}</p></div>`).join('')}`:''}</div>`;
  const linkBox=`<div class="link-box">${icon('link')}<span class="small muted">现场问题提交地址：</span><a target="_blank" rel="noopener" href="${esc(link())}">${esc(link())}</a>${button('复制链接','copy-link',false,'copy')}</div>`;
  return `<div class="section-head"><h2>${live?'活动现场':'现场执行'}</h2><div class="actions">${e.state==='REGISTRATION_OPEN'?button('开始活动','start',true,'radio'):''}${live?button('结束活动','finish',true,'flag'):''}</div></div>${live&&tl?`<div class="inline-note">当前环节：${esc(current?.title||'活动开始前准备')} · ${next?`下一环节：${esc(next.title)}，${Math.max(0,Math.ceil((new Date(next.time)-new Date())/60000))} 分钟后开始`:'时间线已全部结束'}<span class="small muted">（按设备当前时间计算）</span></div>`:''}${linkBox}${pend&&p?`<div class="inline-note warning"><strong>流程调整待确认</strong><p>${esc(pend.reason)} · 从“${esc(p.timeline.find(t=>t.id===pend.item_id)?.title)}”起顺延 ${pend.minutes} 分钟</p><div class="actions">${button('采纳调整','approve_adjustment',true,'check')}${button('暂不调整','reject_adjustment',false,'x')}</div></div>`:''}<div class="section"><div class="section-head"><h2>执行 Timeline</h2>${live&&p&&!pend?button('提出调整','adjust-dialog',false,'clock-3'):''}</div>${tl?`<ul class="timeline">${tl.map(t=>`<li><time>${String(t.time).slice(11,16)}</time><span>${esc(t.title)}<small>${day(t.time)} · ${esc(t.owner)}</small></span></li>`).join('')}</ul>`:'<p class="muted small">开始实施后，将依据策划方案生成执行 Timeline。</p>'}</div><div class="section"><div class="section-head"><h2>现场问题</h2><span class="small muted">${issues.length} 条</span></div>${issues.length?issues.map(f=>`<div class="feedback-item"><p>${esc(f.comment)}</p>${f.suggestion?`<p class="small" style="color:#4f8262">建议：${esc(f.suggestion)}</p>`:''}<span class="small muted">${fmt(f.at)}</span></div>`).join(''):'<p class="muted small">暂无现场问题</p>'}</div><div class="section"><div class="section-head"><h2>现场签到</h2>${button('刷新数据','refresh',false,'refresh-cw')}</div>${registrationTable(e.registrations)}</div>${agentBlock}`;
}
function feedbackList(items=event.feedback_pool) { return items.length?items.map(f=>`<div class="feedback-item"><span class="badge gray">${f.phase==='live'?'现场问题':'复盘反馈'}</span> <span class="rating">${f.rating?'★'.repeat(f.rating):'未评分'}</span><p>${esc(f.comment)}</p>${f.suggestion?`<p class="small" style="color:#4f8262">建议：${esc(f.suggestion)}</p>`:''}<span class="small muted">${fmt(f.at)}</span></div>`).join(''):'<p class="muted small">暂无反馈</p>'; }
function reviewView() {
  const e=event,r=e.review,live=e.feedback_pool.filter(f=>f.phase==='live'),post=e.feedback_pool.filter(f=>f.phase==='post');
  if(!r) return `${e.state==='FEEDBACK'?`<div class="link-box">${icon('link')}<span class="small muted">反馈收集地址：</span><a target="_blank" rel="noopener" href="${esc(link())}">${esc(link())}</a>${button('复制链接','copy-link',false,'copy')}</div>`:''}<div class="section-head"><h2>复盘反馈</h2><span class="small muted">${post.length} 条</span></div>${feedbackList(post)}<div class="section"><div class="section-head"><h2>现场问题</h2><span class="small muted">${live.length} 条</span></div>${feedbackList(live)}</div><div class="section">${empty('让每次活动都有下一次的进步',e.state==='FEEDBACK'?'可基于以上反馈生成复盘报告。':'活动结束后，根据报名、到场和反馈数据生成复盘。',e.state==='FEEDBACK'?button('生成活动复盘','review',true,'sparkles'):'')}</div>`;
  return `<div class="section-head"><h2>活动复盘报告</h2><div class="actions">${e.state==='WAITING_REVIEW_CONFIRMATION'?button('重新生成','review',false,'refresh-cw')+button('确认复盘，生成总结','confirm_review',true,'check'):''}${button('导出报告','report-download',false,'download')}</div></div><p class="prose">${esc(r.summary)}</p><div class="section"><div class="section-head"><h2>目标 vs 实际</h2><span class="small muted">评分样本 ${r.metrics.rating_count} 份</span></div>${comparison(e.plan?.targets||{},r.comparison)}</div><div class="columns section"><div><h2>下一次改进建议</h2><ul class="check-list">${r.suggestions.map(s=>`<li>${esc(s)}</li>`).join('')}</ul><div class="section"><h2>宣传效果</h2><p class="muted small">当前仅记录报名来源，未接入外部平台曝光和阅读数据。</p>${sourceTable()}</div></div><div><h2>参与者评价</h2>${feedbackList(post)}</div></div><div class="section"><h2>执行与调整记录</h2><div class="spacer"></div>${logs(e.logs.filter(l=>['现场','总控'].includes(l.agent)).slice(-8))}</div>${e.recap?`<div class="section">${publicityView(true)}</div>`:''}`;
}
...
function agentPanel() {
  ...
  if(!role)return '';
  const roles=role==='publicity'?['publicity_moments','publicity_article','publicity_xiaohongshu']:[role];
  const setting=agentSettings[role]||{}, e=event;
  let body='';
  const errs=roles.map(r=>agentSettings[r]?.error).filter(Boolean);
  if(errs.length)body+=`<div class="inline-note warning">${esc(errs[0])}</div>`;
  if(role==='publicity'&&['PLAN_CONFIRMED','WAITING_PUBLICITY_CONFIRMATION','REGISTRATION_OPEN'].includes(e.state)&&e.publicity)body+=`<div class="actions">${button('智能修改文案','revise-publicity',false,'sparkles')}</div>`;
...
function modal(html) { $('#modal').innerHTML=`<button class="icon-btn modal-close" data-action="close" aria-label="关闭">${icon('x')}</button>`+html;$('#modal').showModal();refresh(); }
async function streamPlanningChat(data) {
  ...
  const handleFrame = (frame) => {
    ...
    if (name === 'delta') { gotDelta = true; pendingText += payload.text || ''; appendPending(); }
      ...
      if (scroll) {
        scroll.querySelector('.chat-row.pending')?.remove();
        const toolLabels = {write_file: '正在写入文件…', get_plan_skill: '正在读取策划技能…', run_command: '正在执行命令…', read_file: '正在读取文件…', list_files: '正在查看文件列表…', get_event_brief: '正在读取活动信息…', generate_plan: '正在生成活动计划…'};
        scroll.insertAdjacentHTML('beforeend', `<div class="chat-row pending"><div class="chat-bubble tool-status">${icon('loader-circle')}<span>${toolLabels[payload.name] || '正在调用工具…'}</span></div></div>`);
        scroll.scrollTop = scroll.scrollHeight;
        refresh();
      }
    }
...
async function poster(recap=false){
  ...
  wrap(recap?`${event.metrics.registration} 人报名 · ${event.metrics.attendance} 人到场`:(event.brief.objective||event.brief.name),'26px sans-serif',42,790);
  y=Math.max(y+20,850);wrap(fmt(event.brief.date),'24px sans-serif',36,950);wrap(event.brief.location||'地点待定','24px sans-serif',36,950);wrap(event.brief.organizer||'','24px sans-serif',36,1000);
...
document.addEventListener('click',ev=>{
  ...
  if(action==='preview-plan')return modal(`<h2>策划方案预览 <span class="small muted">plan.md · 第 ${event.revision} 版</span></h2><div class="plan-md plan-preview">${md(event.plan_md||'')}</div><div class="form-actions"><span class="small muted" style="margin-right:auto">如需修改，关闭后在对话中告诉我</span>${button('关闭','close',false,'x')}${button('开始实施','confirm_plan',true,'check')}</div>`);
  if(action==='edit-copy'||action==='edit-recap'){const p=action==='edit-copy'?event.publicity:event.recap;return modal(`<h2>编辑宣传文案</h2><form id="copy-form" data-recap="${action==='edit-recap'}"><div class="field"><label>公众号</label><textarea name="article" rows="8" required>${esc(p.article)}</textarea></div><div class="spacer"></div><div class="field"><label>朋友圈</label><textarea name="moments" rows="5" required>${esc(p.moments||p.group||'')}</textarea></div><div class="spacer"></div><div class="field"><label>小红书</label><textarea name="xiaohongshu" rows="5" required>${esc(p.xiaohongshu||'')}</textarea></div><div class="form-actions"><button class="primary" type="submit">保存文案</button></div></form>`);}
  ...
  const confirmations={confirm_plan:['开始实施','确认后将生成报名问卷并开放报名，随后自动生成宣传文案；当前方案将保留快照。'],confirm_publicity:['确认宣传','确认后文案生效，可复制到外部渠道发布；报名入口保持开放。'],start:['开始活动','将关闭报名，开放现场签到与现场问题收集。'],finish:['结束活动','将关闭签到，开放复盘反馈收集。'],confirm_review:['确认复盘结果','将使用当前复盘数据生成活动总结宣传稿。'],confirm_recap:['确认总结并归档','活动将标记为已归档。总结文案需手动发布到外部渠道。'],approve_adjustment:['确认调整正式时间线','正式执行时间线将更新，并保存本次修改记录。']};
  ...
  run(async()=>{
    ...
    else if(action==='copy-content'||action==='copy-recap'){const p=action==='copy-recap'?event.recap:event.publicity;await navigator.clipboard.writeText((p[pubTab]??p.group??'').replaceAll(event.registration_path,link()));toast('文案已复制');}
...
document.addEventListener('submit',ev=>{
  ...
  run(async()=>{
    ...
    if(form.id==='register-form'){await api(`/api/public/${publicId}/register`,data);form.reset();toast('报名成功，期待与你相遇');}
    if(form.id==='feedback-form'){await api(`/api/public/${publicId}/feedback`,data);toast('反馈已保存，感谢参与');}
    if(form.id==='live-form'){await api(`/api/public/${publicId}/feedback`,{comment:data.comment});form.reset();toast('问题已提交，感谢反馈');}
  });
});
document.addEventListener('input',ev=>{if(ev.target.id==='search'){const q=ev.target.value.toLowerCase();$('#registration-table').innerHTML=registrationTable(event.registrations.filter(r=>[r.name,r.contact||r.email||'',r.college||'',JSON.stringify(r.answers||{})].some(v=>v.toLowerCase().includes(q))));refresh();}if(ev.target.id==='planning-message'){ev.target.style.height='auto';ev.target.style.height=Math.min(ev.target.scrollHeight,200)+'px';}});
...
let publicTab='register';
async function loadPublic(tab){publicData=await api('/api/public/'+publicId);renderPublic(tab||'register');}
const DEFAULT_Q={title:'活动报名',intro:'报名信息仅供活动负责人组织活动使用。',fields:[{key:'name',label:'姓名',type:'text',required:true},{key:'contact',label:'联系方式',type:'text',required:true,placeholder:'手机或微信号'},{key:'email',label:'邮箱',type:'email'},{key:'college',label:'学院',type:'text',required:true},{key:'grade',label:'年级',type:'text',placeholder:'如：大二'},{key:'needs',label:'特殊需求',type:'textarea',placeholder:'无障碍、饮食或其他参与需求'},{key:'question',label:'想提前了解什么？',type:'textarea',placeholder:'向负责人提问'},{key:'source',label:'从哪里了解到活动？',type:'select',options:['直接访问','微信公众号','微信群','朋友圈','校园论坛']}]};
function qField(f){
  const label=f.label+(f.required?' *':''),req=f.required?'required':'';
  if(f.type==='textarea')return `<div class="field full"><label for="f-q-${f.key}">${esc(label)}</label><textarea id="f-q-${f.key}" name="${f.key}" ${req} maxlength="1000" placeholder="${esc(f.placeholder||'')}"></textarea></div>`;
  if(f.type==='select')return `<div class="field"><label for="f-q-${f.key}">${esc(label)}</label><select id="f-q-${f.key}" name="${f.key}" ${req}><option value="">请选择</option>${(f.options||[]).map(o=>`<option>${esc(o)}</option>`).join('')}</select></div>`;
  return field(f.key,label,f.type==='textarea'?'text':f.type,'',`${req} maxlength="1000" placeholder="${esc(f.placeholder||'')}"`);
}
function renderPublic(tab){
  if(tab)publicTab=tab;
  if(!publicData)return;
  const e=publicData,b=e.brief;
  const open={register:e.state==='REGISTRATION_OPEN',live:e.state==='LIVE',feedback:e.state==='FEEDBACK'};
  let body='';
  if(publicTab==='live'){
    body=!open.live?empty('现场问题尚未开放','活动开始后可以在这里提出现场问题。'):`<h2>现场问题</h2><p class="muted small">提出的问题会实时同步给现场工作人员。</p><form id="live-form"><div class="field"><label>你的问题或现场反馈</label><textarea name="comment" required maxlength="2000" rows="5" placeholder="例如：音响声音偏小、能否增加提问环节……"></textarea></div><div class="form-actions"><button class="primary" type="submit">${icon('send')}提交</button></div></form>`;
  }else if(publicTab==='feedback'){
    body=!open.feedback?empty('复盘反馈尚未开放','活动结束后开放反馈收集。'):`<h2>复盘反馈</h2><form id="feedback-form"><div class="field"><label>整体满意度 *</label><select name="rating" required><option value="" disabled selected>请选择满意度</option><option value="5">5 · 非常满意</option><option value="4">4 · 满意</option><option value="3">3 · 一般</option><option value="2">2 · 不满意</option><option value="1">1 · 非常不满意</option></select></div><div class="spacer"></div><div class="field"><label>评价、问题或改进建议</label><textarea name="comment" required maxlength="2000" rows="6"></textarea></div><div class="form-actions"><button class="primary" type="submit">${icon('send')}提交反馈</button></div></form>`;
  }else{
    if(!open.register)body=empty(['DRAFT','WAITING_PLAN_CONFIRMATION','PLAN_CONFIRMED','WAITING_PUBLICITY_CONFIRMATION'].includes(e.state)?'当前未开放报名':'报名已结束',labels[e.state]);
    else body=(()=>{const q=e.questionnaire||DEFAULT_Q;return `<div class="section-head"><h2>${esc(q.title||'活动报名')}</h2>${e.remaining!=null?`<span class="badge">剩余 ${e.remaining} 个名额</span>`:''}</div><p class="muted small">${esc(q.intro||'')}</p><form id="register-form"><div class="form-grid">${q.fields.map(qField).join('')}</div><div class="form-actions"><button class="primary" type="submit" ${e.remaining===0?'disabled':''}>${icon('send')}提交报名</button></div></form>`;})();
  }
  $('#app').innerHTML=`<main class="public-shell"><header class="public-top"><div class="brand"><span class="brand-mark">${icon('sprout')}</span>校园共创</div><button class="icon-btn" data-action="refresh" title="刷新活动状态">${icon('refresh-cw')}</button></header><div class="public-content"><div class="public-head">${badge(e.state)}<h1>${esc(b.name)}</h1><div class="event-meta"><span>${icon('calendar-days')}${fmt(b.date)}</span><span>${icon('map-pin')}${esc(b.location||'待确定')}</span></div></div><div class="tabbar">${[['register','活动报名'],['live','现场问题'],['feedback','复盘反馈']].map(([id,title])=>`<button data-public-tab="${id}" class="${publicTab===id?'active':''}" ${open[id]?'':'disabled'}>${title}</button>`).join('')}</div>${body}</div></main>`;refresh();
}
