...
function publicityView(recap=false) {
  ...
  const editable=recap?e.state==='WAITING_RECAP_CONFIRMATION':(e.state==='WAITING_PUBLICITY_CONFIRMATION'||(e.state==='REGISTRATION_OPEN'&&!p.approved));
  return `${editable?'<div class="inline-note warning">文案待确认。本站不直接连接微信公众号或社交平台，确认后可复制文案与下载海报。</div>':'<div class="inline-note">文案已确认，可复制到外部渠道发布。</div>'}<div class="section-head"><h2>${recap?'活动总结宣传':'活动宣传素材'}</h2><div class="actions">${editable?button('修改文案',recap?'edit-recap':'edit-copy',false,'pencil')+button(recap?'确认总结并归档':'确认宣传',recap?'confirm_recap':'confirm_publicity',true,'check'):''}</div></div><div class="split"><div><div class="tabbar">${[['article','公众号'],['moments','朋友圈'],['xiaohongshu','小红书']].map(([id,t])=>`<button class="${pubTab===id?'active':''}" data-tab="${id}">${t}</button>`).join('')}</div><div class="prose">${esc(p[pubTab]??p.group??'').replaceAll(esc(e.registration_path),esc(link()))}</div><div class="form-actions">${button('复制文案',recap?'copy-recap':'copy-content',false,'copy')}</div><div class="section"><h2>宣传节奏</h2><ul class="timeline">${p.schedule.map((s,i)=>`<li><time>0${i+1}</time><span>${esc(s)}</span></li>`).join('')}</ul></div></div><div><div class="poster"><div class="poster-photo"></div><span class="eyebrow">CAMPUS TOGETHER / ${recap?'活动回顾':'校园活动'}</span><h2>${esc(e.brief.name)}</h2><p>${recap?`${e.metrics.registration} 人报名 · ${e.metrics.attendance} 人到场` :esc(e.brief.objective)}</p><p>${fmt(e.brief.date)}<br>${esc(e.brief.location)}<br>${esc(e.brief.organizer)}</p><canvas id="qr" aria-label="活动入口二维码"></canvas><span class="small">${recap?'活动参与者入口':'扫码报名 · 期待相遇'}</span></div><div class="form-actions">${button('下载海报',recap?'poster-recap':'poster',false,'download')}</div></div></div>`;
}
function registrationView() {
  if (!event.registration_path) return empty('报名入口尚未创建','确认策划后，由系统自动创建入口。');
  return `<div class="link-box">${icon('link')}<a target="_blank" rel="noopener" href="${esc(link())}">${esc(link())}</a>${button('复制链接','copy-link',false,'copy')}${button('二维码','show-qr',false,'qr-code')}</div>${event.questionnaire?`<p class="small muted" style="margin:-12px 0 16px">报名问卷（${event.questionnaire.fields.length} 个字段）：${event.questionnaire.fields.map(f=>esc(f.label)).join('、')}</p>`:''}${event.state==='PLAN_CONFIRMED'||event.state==='WAITING_PUBLICITY_CONFIRMATION'?'<div class="inline-note warning">报名入口已创建，宣传确认后开放提交。</div>':''}${metricCards(event)}<div class="section-head"><h2>报名人员 <span class="small muted">${event.registrations.length} 人</span></h2><div class="actions"><input class="search" id="search" aria-label="搜索报名人员" placeholder="搜索姓名、邮箱、学院">${button('导出名单','export',false,'download')}${event.state==='REGISTRATION_OPEN'?button('开始活动','start',true,'radio'):''}</div></div><div id="registration-table">${registrationTable(event.registrations)}</div><div class="columns section"><div><h2>近 7 天报名趋势</h2>${trend()}</div><div><h2>学院分布</h2>${distribution()}</div></div>`;
}
...
function distribution() {
  const map={};event.registrations.forEach(r=>{if(r.college)map[r.college]=(map[r.college]||0)+1;});
