...
function agentPanel() {
  ...
  body+=`<div class="actions" style="margin-bottom:16px"><a href="/agents/${role}">独立测试 ${icon('arrow-up-right')}</a></div>`;
...
async function settingsModal(){
  const [settings,runs]=await Promise.all([api('/api/agents'),api('/api/agent-runs')]);agentSettings=settings;
...
function example(){const d=new Date();d.setDate(d.getDate()+7);const date=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}T19:00`;const values={name:'AI 创新交流夜',objective:'认识 AI Agent 的实际应用，促进跨学院交流',type:'交流分享',level:'校级',format:'线下分享与互动讨论',audience:'全校对 AI 感兴趣的同学',date,duration:120,location:'大学生活动中心 · 201 报告厅',capacity:100,budget:1200,organizer:'学生科技协会',owner:'活动负责人',constraints:'21:00 前结束；欢迎零基础同学参加。'};for(const [k,v]of Object.entries(values))$('#plan-form').elements[k].value=v;}
...
document.addEventListener('click',ev=>{
  ...
  if(target.dataset.testAgent){run(async()=>{await api(`/api/agents/${target.dataset.testAgent}/test`,{});toast('模型接口连接与 JSON 输出测试通过');await settingsModal();});return;}
  ...
  if(action==='agent-settings')return run(settingsModal);
