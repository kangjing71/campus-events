...
function onsiteView() {
  ...
  const current=live&&p?[...p.timeline].reverse().find(t=>new Date(t.time)<=new Date()):null;
  const next=live&&p?p.timeline.find(t=>new Date(t.time)>new Date()):null;
  return `<div class="section-head"><h2>${live?'活动现场':'现场执行记录'}</h2><div class="actions">${e.state==='REGISTRATION_OPEN'?button('开始活动','start',true,'radio'):''}${live?button('结束活动','finish',true,'flag'):''}</div></div>${live&&p?`<div class="inline-note">当前环节：${esc(current?.title||'活动开始前准备')} · ${next?`下一环节：${esc(next.title)}，${Math.max(0,Math.ceil((new Date(next.time)-new Date())/60000))} 分钟后开始`:'时间线已全部结束'}<span class="small muted">（按设备当前时间计算）</span></div>`:''}${pend&&p?`<div class="inline-note warning"><strong>流程调整待确认</strong><p>${esc(pend.reason)} · 从“${esc(p.timeline.find(t=>t.id===pend.item_id)?.title)}”起顺延 ${pend.minutes} 分钟</p><div class="actions">${button('采纳调整','approve_adjustment',true,'check')}${button('暂不调整','reject_adjustment',false,'x')}</div></div>`:''}<div class="columns"><div><div class="section-head"><h2>执行 Timeline</h2>${live&&p&&!pend?button('提出调整','adjust-dialog',false,'clock-3'):''}</div>${p?timeline(p.timeline):'<p class="muted small">策划方案为文档形式，执行时间线暂未对接。</p>'}</div><div><div class="section-head"><h2>参与者反馈</h2><span class="small muted">${e.feedback_pool.length} 条</span></div>${feedbackList()}</div></div><div class="section"><div class="section-head"><h2>现场签到</h2>${button('刷新数据','refresh',false,'refresh-cw')}</div>${registrationTable(e.registrations)}</div><div class="section"><h2>现场调整记录</h2><ul class="timeline">${e.adjustments.map(a=>`<li><time>+${a.minutes} 分钟</time><span>${esc(a.reason)}<small>${fmt(a.at)} · 已批准</small></span></li>`).join('')||'<li><span class="muted">暂无流程调整</span></li>'}</ul></div>`;
}
...
document.addEventListener('submit',ev=>{
  ...
  run(async()=>{
    ...
    if(form.id==='live-form'){await api(`/api/public/${publicId}/feedback`,{comment:data.comment,ticket:ticket()});form.reset();toast('问题已提交，感谢反馈');}
  });
});
...
async function loadPublic(tab){publicData=await api('/api/public/'+publicId);ticketInfo=null;if(ticket()){try{ticketInfo=await api(`/api/public/${publicId}/ticket`,{ticket:ticket()});}catch{localStorage.removeItem('ticket-'+publicId);}}renderPublic(tab||'register');}
...
function renderPublic(tab){
  ...
  let body='';
  if(publicTab==='ticket'){
    body=ticketInfo?ticketCard(e):`<h2>查询报名凭证</h2><form id="ticket-form">${field('ticket','报名凭证','text','','required')}<div class="form-actions"><button class="primary" type="submit">查看报名</button></div></form>`;
  }else if(publicTab==='live'){
    body=e.state!=='LIVE'?empty('现场问题尚未开放','活动开始后可以在这里提出现场问题。'):!ticketInfo?empty('请先完成报名与签到','在「活动报名」页报名，活动当天在现场签到后即可提问。'):!ticketInfo.checked_at?empty('请先完成现场签到','在「活动报名」页出示报名凭证完成签到后即可提问。'):`<h2>现场问题</h2><p class="muted small">提出的问题会实时同步给现场工作人员。</p><form id="live-form"><div class="field"><label>你的问题或现场反馈</label><textarea name="comment" required maxlength="2000" rows="5" placeholder="例如：音响声音偏小、能否增加提问环节……"></textarea></div><div class="form-actions"><button class="primary" type="submit">${icon('send')}提交</button></div></form>`;
  }else if(publicTab==='feedback'){
    body=e.state!=='FEEDBACK'?empty('复盘反馈尚未开放','活动结束后开放反馈收集。'):!ticketInfo?empty('请先完成报名与签到','报名并签到后可提交反馈。'):!ticketInfo.checked_at?empty('请先完成现场签到','签到成功后可提交活动反馈。'):`<h2>复盘反馈</h2><form id="feedback-form"><div class="field"><label>整体满意度</label><select name="rating"><option value="">仅提交文字反馈</option><option value="5">5 · 非常满意</option><option value="4">4 · 满意</option><option value="3">3 · 一般</option><option value="2">2 · 不满意</option><option value="1">1 · 非常不满意</option></select></div><div class="spacer"></div><div class="field"><label>评价、问题或改进建议</label><textarea name="comment" required maxlength="2000" rows="6"></textarea></div><div class="form-actions"><button class="primary" type="submit">${icon('send')}提交反馈</button></div></form>`;
  }else{
    if(ticketInfo)body=ticketCard(e);
    else if(e.state==='REGISTRATION_OPEN')body=(()=>{const q=e.questionnaire||DEFAULT_Q;return `<div class="section-head"><h2>${esc(q.title||'活动报名')}</h2>${e.remaining!=null?`<span class="badge">剩余 ${e.remaining} 个名额</span>`:''}</div><p class="muted small">${esc(q.intro||'')}</p><form id="register-form"><div class="form-grid">${q.fields.map(qField).join('')}</div><div class="form-actions"><button class="primary" type="submit" ${e.remaining===0?'disabled':''}>${icon('send')}提交报名</button></div></form>`;})();
    else body=empty(['DRAFT','WAITING_PLAN_CONFIRMATION','PLAN_CONFIRMED','WAITING_PUBLICITY_CONFIRMATION'].includes(e.state)?'当前未开放报名':'报名已结束',labels[e.state]);
    if(!ticketInfo)body+=`<p class="small muted" style="margin-top:16px;text-align:center"><button class="linklike" data-public-tab="ticket">已报名？查询报名凭证</button></p>`;
  }
  $('#app').innerHTML=`<main class="public-shell"><header class="public-top"><div class="brand"><span class="brand-mark">${icon('sprout')}</span>校园共创</div><button class="icon-btn" data-action="refresh" title="刷新活动状态">${icon('refresh-cw')}</button></header><div class="public-content"><div class="public-head">${badge(e.state)}<h1>${esc(b.name)}</h1><div class="event-meta"><span>${icon('calendar-days')}${fmt(b.date)}</span><span>${icon('map-pin')}${esc(b.location||'待确定')}</span></div></div><div class="tabbar">${[['register','活动报名'],['live','现场问题'],['feedback','复盘反馈']].map(([id,title])=>`<button data-public-tab="${id}" class="${publicTab===id?'active':''}">${title}</button>`).join('')}</div>${body}</div></main>`;refresh();
}
